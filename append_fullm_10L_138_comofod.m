...
list = dir('D:\CNNpaper\Waveletbusternet\data\0riginal\Test01\Test\Forgery\*'); 
numfiles = size(list,1);
%o = 1005;
for o = 1:numfiles %Grip10번파일 40번부터 엑셀값다시 - g해결햇음
    
    fn = strcat('D:\CNNpaper\Waveletbusternet\data\0riginal\Test01\Test\Forgery','\',list(o).name);
...
%imagename = strcat(int2str(o),'.jpg');

name = strcat('C:\Users\JYP\Desktop\1\result1','\',imagename);
%imwrite(final_image,name);
%% FPR, TPR 나중에옮길것
gtlist = dir('D:\CNNpaper\Waveletbusternet\data\0riginal\Test01\Test\GroundTruth\*.png'); %jpeg 5는jpg
gtfn = strcat('D:\CNNpaper\Waveletbusternet\data\0riginal\Test01\Test\GroundTruth','\',gtlist(o).name);
gt = imread(gtfn);
%gt = rgb2gray(gt);
fi = final_image;
gt = im2bw(gt,0.5);
...
Pricision = TP/(TP+FP)*100;
Recall = TP/(FN+TP)*100;
F_score = 2 * (Pricision*Recall) / (Pricision+Recall);

final_Result(o,1:6) = [Pricision, Recall, F_score, TPR, FPR, ACC];  
remissing_final = rmmissing(final_Result);
end
filename = 'SIFT_Cslbp01.xlsx';
xlswrite(filename,final_Result,1);
filename = 'remissing_SIFT_Cslbp01.xlsx';
xlswrite(filename,remissing_final,1);