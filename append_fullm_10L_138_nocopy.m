...
list = dir('C:\Users\Jun Young\Desktop\ImageData\Coverage\Ori\*.tif');  %/comofod/2000/220 폴더변경
...
for o = 1:numfiles
    ...
    fn = strcat('C:\Users\Jun Young\Desktop\ImageData\Coverage\Ori','\',list(o).name);
    %IM = imread(fn);

    [image, descriptors, locs] = sift(fn);
    %showkeys(image, locs);
    %num = match('DSC_1568tamp1.jpg');
    fdes = descriptors(:,1:128);
    loc5 = round(locs(:,1:2));
    locs = locs(:,1:2);
    img1 = imread(fn);
    img111 = rgb2gray(img1);
    [cols, rows]=size(img111);
...
%%    
    ...
    if size(Bp1,2) < 3 %%열갯수 = 나온매칭수
        continue
    end
    
    %%cluster 아니고, 좌표 정리 해주는과정 두 지역으로 나누는과정
%     cen_mat = [];
%     
%     for i = 1:size(Bp1,2)
...
% 
%     T = 30; %이상하게 나온거 변경 %2번그림 작게30, 2000 -> T=500
%     nBp1_temp = [];
%     nBp2_temp = [];
% 
...
name = strcat('C:\Users\Jun Young\Desktop\ImageData\Coverage\Ori\result','\',imagename);
